    ...
    try {
        ...
        if (shouldInitOnboarding) {
            ...
            try {
                ...
                console.error("Failed to open the onboarding screen:", onboardingError);
            }
        }
        ...
        if (!isStaleAuthSession() && !state.scoresHydrated && !(data && data.scores)) {
            try {
                ...
                ScoreManager.loadScores();
                state.scoresHydrated = true;
                state.scoresDirty = Object.keys(state.savedScores || {}).length > 0;
                ScoreManager.queuePendingScoreSave();
            } catch (hydrateError) {
                console.error("Score fallback hydration failed:", hydrateError);
            }
        }
        return null;
    }
