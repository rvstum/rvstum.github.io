...
import { initAuthLifecycle } from "./authLifecycle.js?v=20260921-bk-title-color-2";
import { initOnboardingUI } from "./onboardingUI.js?v=20260311-profile-original-sync-1";
import { handleProfileLink } from "./routeManager.js?v=20260921-bk-title-color-2";
import { exitViewMode as runExitViewMode } from "./viewModeExit.js?v=20260921-bk-title-color-2";
import { initProfileModalController } from "./profileModalUI.js?v=20260403-loader-visible-idempotent-2";
...
function clearBenchmarkVisualState() {
    ...
    document.querySelectorAll(".row-rating-value").forEach((valueEl) => {
        valueEl.textContent = "0";
        valueEl.style.color = "white";
    });
    document.querySelectorAll(".ranks-bars").forEach((row) => {
        row.style.removeProperty("--row-rank-strip");
        row.classList.remove("has-rank-strip");
    });
...
function syncMobileBenchmarkGeometry() {
    ...
    let extentTop = Infinity;
    let extentBottom = -Infinity;
    extentEls.forEach((el) => {
        const r = el.getBoundingClientRect();
        if (r.height <= 0) return;
        extentTop = Math.min(extentTop, r.top);
        extentBottom = Math.max(extentBottom, r.bottom);
    });
    const rankBoxCenter = rankBoxParent && Number.isFinite(extentTop) && Number.isFinite(extentBottom)
        ? ((extentTop + extentBottom) / 2) - rankBoxParent.getBoundingClientRect().top
        : null;
    ...
    if (Number.isFinite(rankBoxCenter) && rankBoxCenter > 0) {
        body.style.setProperty('--mobile-rank-box-center', `${Math.round(rankBoxCenter)}px`);
    }
...
function initStartupSideEffects() {
    ...
    window.addEventListener('orientationchange', syncMobileLayoutState, { passive: true });
    window.addEventListener('pageshow', syncMobileLayoutState, { passive: true });
    window.addEventListener('load', () => {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', syncMobileLayoutState);
    } else {
        syncMobileLayoutState();
    }
}
