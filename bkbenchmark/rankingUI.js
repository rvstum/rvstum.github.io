...
function updateRankTierTrack(rankBox, rankIndex, progressInRank, isComplete) {
    const track = rankBox.querySelector('.rank-tier-track');
    if (!track) return;
    ...
    else if (p < 80) fill = p * 1.25;
    else fill = 100 + ((p - 80) / 20) * 5;
    track.style.setProperty('--tier-fill', fill + '%');
    track.querySelectorAll('.rank-tier-step').forEach((step, i) => {
        step.classList.toggle('is-active', i === activeIdx);
        step.classList.toggle('is-passed', i < activeIdx);
    });
}
...
function positionRankDivider() {
    ...
    const firstRank = document.querySelector('.ranks-labels .rank-item');
    if (!lastStep || !firstRank) return;
        ...
        if (currentRankIndex === 0) {
            rankBox.innerHTML = `
                <div class="rank-up-content">
                    <div class="rank-up-info">
                        <div class="rank-up-label" data-label="${tForLang(currentLanguage, 'rank_current_label')}"></div>
                        <span class="rank-up-text rank-up-text-base">${name}</span>
                        ${buildRankTierTrack()}
                    </div>
                </div>`;
            updateRankTierTrack(rankBox, 0, progressInRank, false);
            rankBox.style.color = '';
            ...
            switch(currentRankIndex) {
                ...
                case 11: 
                    ...
                    textStyle = `background: linear-gradient(110deg, #FF6F00 20%, #FF8F00 40%, #FFA000 48%, #FFB300 50%, #FFA000 52%, #FF8F00 60%, #FF6F00 80%); background-size: 200% auto; -webkit-background-clip: text; background-clip: text; color: transparent; -webkit-text-fill-color: transparent; animation: eternalShimmer 2.5s linear infinite; animation-delay: ${eternalAnimationDelay};`;
                    break;
                case 12: 
                    ...
                    textStyle = `background: linear-gradient(110deg, #D8007F 20%, #E91E63 35%, #F06292 45%, #FF80AB 50%, #F06292 55%, #E91E63 65%, #D8007F 80%); background-size: 200% auto; -webkit-background-clip: text; background-clip: text; color: transparent; -webkit-text-fill-color: transparent; animation: eternalShimmer 2.5s linear infinite; animation-delay: ${eternalAnimationDelay};`;
                    break;
                case 13: 
                    ...
                    textStyle = `background: linear-gradient(110deg, #cab98a 20%, #e5d9b6 35%, #f2e9cf 48%, #fff7e5 50%, #f2e9cf 52%, #e5d9b6 65%, #cab98a 80%); background-size: 200% auto; -webkit-background-clip: text; background-clip: text; color: transparent; -webkit-text-fill-color: transparent; animation: eternalShimmer 2.5s linear infinite; animation-delay: ${eternalAnimationDelay};`;
                    break;
            }
            ...
            updateRankTierTrack(rankBox, currentRankIndex, progressInRank, totalRating >= maxRating);
        }

        if (shouldAnimateRankUp) {
            rankBox.classList.remove('rank-up');
        }
        requestAnimationFrame(positionRankDivider);
    }
...
function getRowRankIndex(rating) {
    return rating >= 100 ? Math.min(FINAL_RANK_INDEX, Math.floor(rating / 100)) : 0;
}

function renderRowRatingValues() {
    document.querySelectorAll('.row-rating-value').forEach((div) => {
        const rowIndex = Number(div.dataset.rowIndex);
        const rating = state.individualRatings[rowIndex] || 0;
        div.textContent = rating;
        div.style.color = rating > 0 ? SCORE_TEXT_COLORS[getRowRankIndex(rating)] : 'white';
    });
}
...
function updateRowStrips() {
    document.querySelectorAll('.ranks-bars').forEach((row, rowIndex) => {
        ...
        row.classList.toggle('has-rank-strip', rankIndex > 0);
        if (rankIndex > 1) {
            row.style.setProperty('--row-rank-strip', `linear-gradient(to right, transparent, ${hexToRgba(RANK_COLORS[rankIndex], 0.3)})`);
        } else if (rankIndex === 1) {
            row.style.setProperty('--row-rank-strip', 'linear-gradient(to right, transparent, rgba(255, 255, 255, 0.1))');
        } else {
            row.style.removeProperty('--row-rank-strip');
        }
    });
}
