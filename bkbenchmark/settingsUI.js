...
function bindSettingSwitch(select) {
    ...
    button.innerHTML = '<span class="settings-switch-thumb"></span>';
    const labelledBy = select.id ? document.querySelector('label[for="' + select.id + '"]') : null;
    const ariaLabel = select.getAttribute("aria-label") || (labelledBy ? labelledBy.textContent.trim() : "");
...
function applyThresholdVisualization(enabled) {
    document.body.classList.toggle("hide-threshold-diamonds", !enabled);
}
...
export function initSettingsUI(options = {}) {
    ...
    document.querySelectorAll("select[data-settings-switch]").forEach(bindSettingSwitch);
    const thresholdVizSelect = getCachedElementById("rankThresholdVizSelect");
    const thresholdVizToggle = getCachedElementById("rankThresholdVizToggle");
    const syncThresholdVizToggle = () => {
        if (!thresholdVizToggle || !thresholdVizSelect) return;
        const on = thresholdVizSelect.value === "on";
        thresholdVizToggle.dataset.state = on ? "on" : "off";
        thresholdVizToggle.setAttribute("aria-pressed", on ? "true" : "false");
    };
    if (thresholdVizToggle && thresholdVizSelect) {
        thresholdVizToggle.addEventListener("click", () => {
            thresholdVizSelect.value = thresholdVizSelect.value === "on" ? "off" : "on";
            thresholdVizSelect.dispatchEvent(new Event("change", { bubbles: true }));
        });
        thresholdVizSelect.addEventListener("change", syncThresholdVizToggle);
    }
    if (thresholdVizSelect) {
        thresholdVizSelect.value = readString(THRESHOLD_VIZ_STORAGE_KEY) === "on" ? "on" : "off";
        syncThresholdVizToggle();
        applyThresholdVisualization(thresholdVizSelect.value === "on");
        thresholdVizSelect.addEventListener("change", () => {
            writeString(THRESHOLD_VIZ_STORAGE_KEY, thresholdVizSelect.value);
            applyThresholdVisualization(thresholdVizSelect.value === "on");
        });
    }
