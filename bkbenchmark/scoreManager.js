...
function isScoreResetPending() {
    return readString(SCORE_RESET_PENDING_STORAGE_KEY, "false") === "true";
}

function clearScoreSaveRetry() {
    if (scoreSaveRetryTimer) clearTimeout(scoreSaveRetryTimer);
    scoreSaveRetryTimer = null;
}

function scheduleScoreSaveRetry() {
    if (scoreSaveRetryTimer || scoreSaveRetryCount >= SCORE_SAVE_RETRY_DELAYS_MS.length) return;
    const delay = SCORE_SAVE_RETRY_DELAYS_MS[scoreSaveRetryCount];
    scoreSaveRetryCount += 1;
    scoreSaveRetryTimer = setTimeout(() => {
        scoreSaveRetryTimer = null;
        if (state.isViewMode || !state.scoresDirty) return;
        saveSavedScores({ resetIntent: isScoreResetPending() }).catch(console.error);
    }, delay);
}

export async function flushDirtyScores() {
    if (state.isViewMode || !state.scoresDirty || !isSignedInUser()) return false;
    clearScoreSaveRetry();
    return saveSavedScores({ resetIntent: isScoreResetPending() });
}
...
if (typeof document !== "undefined") {
    ...
    window.addEventListener("online", () => {
        scoreSaveRetryCount = 0;
        flushDirtyScores().catch(console.error);
    });
}
