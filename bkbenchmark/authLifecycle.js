...
import { getFriendGraph, onFriendGraphChange, startFriendGraph, stopFriendGraph } from "./friendGraphStore.js?v=20260920-friend-graph";
...
function redirectToLoginIfSignedOut() {
    ...
    if (params.get("id") || Slugs.getRequestedProfileSlugFromPath()) return false;
    const loginUrl = Slugs.getBenchmarkLoginUrl();
    const trimSlashes = (value) => value.replace(/\/+$/, "");
    const currentPath = trimSlashes(window.location.pathname || "");
    const loginPath = trimSlashes(new URL(loginUrl, window.location.href).pathname);
...
export function initAuthLifecycle(options = {}) {
    ...
    const refreshFriendRequestBadge = () => {
        ...
        if (!graph) return;
        const incomingUids = normalizeFriendRequestIds(graph.incoming.map((request) => request && request.fromUid));
        ...
        AuthManager.syncFriendRequestState(currentUser.uid, incomingUids, requestsTabActive);
        updateNotificationVisibility();
    };
    onFriendGraphChange(refreshFriendRequestBadge);
    ...
    onAuthStateChanged(auth, async (user) => {
        ...
        if (user) {
            ...
            unsubscribeUserSnapshot = onSnapshot(
                ...
                (docSnap) => {
                    AuthManager.syncLiveProfileViewCount(user.uid, docSnap.data());
                },
                (userSnapshotErr) => {
                    console.warn("Live profile snapshot unavailable:", userSnapshotErr);
                }
            );
            startFriendGraph(user.uid);
        ...
        if (AuthManager.isLoginHandoffPending()) {
            pendingInitialSignedOutHideTimer = setTimeout(() => {
                ...
                AuthManager.clearLoginAuthBootstrapPending();
                AuthManager.clearLoginRestoreBootstrapPending();
                if (redirectToLoginIfSignedOut()) return;
                hidePageLoader({ forceReleaseBootHold: true });
